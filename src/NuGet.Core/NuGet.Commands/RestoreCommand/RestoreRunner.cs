    ...
    public static class RestoreRunner
    {
            ...
            CancellationToken token)
        {
            ...
            var restoreSummaries = await ConcurrencyUtilities.RunAsync(
                    restoreRequests.Select(request => new Func<Task<RestoreSummary>>(() => ExecuteAndCommitAsync(request, token))),
                    maxTasks,
                    useTaskRun: true);
            ...
            RestoreArgs restoreContext)
        {
            ...
            var restoreResults = await ConcurrencyUtilities.RunAsync(
                    restoreRequests.Select(request => new Func<Task<RestoreResultPair>>(() => ExecuteAsync(request, CancellationToken.None))),
                    maxTasks,
                    useTaskRun: true);
